...
lje.vm = {}--> Loads a script file relative to the current script's directory. Optionally controls whether the script is executed immediately after loading. Defaults to executing if the second argument is omitted. Requires an active script context. Load and runtime errors are printed to the console rather than raised, in which case nothing is returned.
...
--> Searches for files matching `search_path` relative to the current script's directory. Returns `nil` if no script is active.Supports wildcard patterns, so if you wanted to find all files in `detours/`, you would do `detours/*`.
...
--> Returns the internal LuaJIT fast function ID (`ffid`) of a function.- `0` means a Lua function- `1` means a C function- Anything else is the unique ID of a fast function.
...
--> A `string.format`-style console print with inline ANSI color support. Wrap text in `$colorName{...}` to colorize it.  Supported colors: `black`, `red`, `green`, `yellow`, `blue`, `magenta`, `cyan`, `white`, `default`.  Example: ```lua lje.con_printf("$red{Error}: %s", message) ```
...
--> Resolves a dot-separated path in the host state's global environment using raw lookups (no metamethods) and brings the resulting value into the secure state.  - Paths starting with `_R.` are resolved in the host registry instead of the global environment (e.g. `_R.Entity`). - If the final value is a **C function**, it is wrapped in a secure dispatcher: calling the wrapper executes the original C function against the host state while redirecting its Lua C API activity (e.g. registry access) back to the secure state. Lua functions are rejected and `nil` is returned. - Any other value is deep-copied into the secure state (see `lje.proxy.copy` for the copy semantics).  Returns `nil` if any path segment is missing, empty, or a non-table intermediate. Each failure case is logged to the console.
...
--> Reads a single setting from the current script's merged settings, by dotted path.The key is traversed through nested tables, so `get("render.color")` returns `settings.render.color`. If any segment along the path is missing (or is not a table), `default` is returned when provided, otherwise `nil`.
...
--> Binds a settings object to the **calling** script and returns it. Call this once at load time (while your script has an active context) and reuse the returned object anywhere — including deferred callbacks such as hooks, render, and timers, where the bare `get`/`all`/`reload` would not know which script is asking.The object exposes `:get(key, default)`, `:all()`, and `:reload()`, which behave like the namespace functions but always resolve to the script that opened them.
--- @return userdata | nil
function lje.settings.open() end --- @diagnostic disable-line
--> Lower-level primitive behind `lje.settings.open()`. Returns a settings object bound to the script with the given name (its folder name, as returned by `lje.env.current_script()`).Most scripts should use `lje.settings.open()` instead, which captures the calling script's name for you.
...
--> Begins a path into `target`, rooted at the global named `root`. Returns a **path object** — an inert builder. Each builder method appends one operation and returns the same object, so calls chain. Nothing reads from `target` until you call `:copy()`.`root` is looked up by raw key in the target state's global environment (no metamethods). It is a single global name, **not** a dotted path — use `:index` to descend further.### Path methodsEach returns the path object for chaining, except `:copy()`, which evaluates the chain.| Method | Effect || --- | --- || `:index(key)` | Raw-indexes the current value (which must be a table) by the string `key`. || `:upvalue(n)` | Reads the `n`-th upvalue (1-based) of the current value, which must be a Lua or C function. Fast/builtin functions have no accessible upvalues and fail. || `:expect(type)` | Asserts the current value's type and fails the path otherwise. Accepts Lua type names (`"table"`, `"function"`, `"userdata"`, …) and GMod metatypes by `MetaName` (`"Player"`, `"Vector"`, …), resolved from the object's metatable. GMod metatype takes priority over the bare Lua type. Opt-in — only add it where you want the guard. || `:copy()` | Evaluates the whole chain against `target` and deep-copies the final value into the secure state. Returns the copy, or `nil` if any step fails (each failure is logged to the console). See `lje.proxy.copy` for the copy semantics. |If the root is missing or `nil`, or any operation fails — indexing a non-table, a missing key, taking an upvalue of a non-function or an out-of-range index, or a failed `:expect` — `:copy()` logs the reason and returns `nil`.### Example```lua-- Pull the `teams` table that team.GetName closes over, without ever-- holding a reference into the client state.local teams = lje.state.path(lje.state.client, "team")  :index("GetName")  -- team.GetName (a function)  :upvalue(1)        -- its first upvalue  :expect("table")   -- guard: make sure it really is a table  :copy()            -- deep-copy it into the secure state```
...
--> Registers a per-script engine call hook. Whenever the engine invokes a Lua function in the host (game) state, the callback is invoked in the secure state as `fn(func, nargs, nresults, ...)`:  - `func` — a light userdata pointer identifying the called function. Useful for identity comparisons without copying a full function object across states. - `nargs` / `nresults` — the argument and result counts of the engine call. - `...` — the call arguments. Tables and userdata arrive as proxy handles (see `lje.proxy`); other values are copied into the secure state.  The hook's return values are ignored — handling/suppressing engine calls is not currently supported. Errors thrown by the hook are caught and printed to the console. Requires an active script context, and hooks only fire for secure scripts. Only one hook can be active per script at a time.
--- @param fn function Must be a Lua function, not a C function.
--- @return nil
function lje.vm.set_engine_call_hook(fn) end --- @diagnostic disable-line
--> Controls when this script's engine call hook runs relative to the engine call itself.  - `false` (default) — the hook runs *before* the engine call proceeds normally. - `true` — LJE performs the engine call first, then invokes the hook afterwards, so the hook observes a call that has already happened. Since the call is consumed in the process, any remaining scripts' hooks are skipped for that call, and if the hook itself errors, the call's status is returned to the engine as-is.  Requires an active script context.
